...
const SvgStreamComponent = React.createClass({
  propTypes: {
  ...
  componentWillMount () {
    ...
    let s = 0.39 / Math.max((b[1][0] - b[0][0]) / this.width, (b[1][1] - b[0][1]) / this.height)
        ...
        <g key={tributary.properties.gid} className={accessClass}>
          ...
          <g>
            ...
            <text 
              ...
              className={classes.radialText} 
              x={RADIUS + 36}
              dy='0.35em'>{point.stream.properties.name} </text>
          </g>
        </g>
        )
          ...
          <g clipPath='url(#circle-stencil)'>
            ...
            <g>
            {
              this.props.tributaries.map((geoJson, sectionIndex) => {
                let section = geoJson.properties.stream
...
export default SvgStreamComponent
